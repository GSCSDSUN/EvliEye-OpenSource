...
import gg.evlieye.util.ListWidget;
...
public final class KeybindManagerScreen extends Screen
{
	...
	public void init()
	{
		...
		addDrawableChild(ButtonWidget.builder(Text.literal("Reset Keybinds"),
			b -> client.setScreen(new ConfirmScreen(confirmed -> {
				if(confirmed)
					EvlieyeClient.INSTANCE.getKeybinds()
	...
	private void edit()
	{
		Keybind keybind = EvlieyeClient.INSTANCE.getKeybinds().getAllKeybinds()
	...
	private void remove()
	{
		Keybind keybind1 = EvlieyeClient.INSTANCE.getKeybinds().getAllKeybinds()
			.get(listGui.selected);
		EvlieyeClient.INSTANCE.getKeybinds().remove(keybind1.getKey());
	}
	...
	private static final class ListGui extends ListWidget
	{
		...
		{
			return EvlieyeClient.INSTANCE.getKeybinds().getAllKeybinds().size();
		}
		...
		{
			Keybind keybind =
				EvlieyeClient.INSTANCE.getKeybinds().getAllKeybinds().get(index);
