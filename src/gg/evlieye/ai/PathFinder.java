...
public class PathFinder
{
	...
	public PathFinder(BlockPos goal)
	{
		if(EvlieyeClient.MC.player.isOnGround())
			start = new PathPos(BlockPos.ofFloored(EvlieyeClient.MC.player.getX(),
				EvlieyeClient.MC.player.getY() + 0.5,
				EvlieyeClient.MC.player.getZ()));
		else
			start =
				new PathPos(BlockPos.ofFloored(EvlieyeClient.MC.player.getPos()));
		this.goal = goal;
	...
	private ArrayList<PathPos> getNeighbors(PathPos pos)
	{
		...
		// up
		if(pos.getY() < EvlieyeClient.MC.world.getTopY() && canGoThrough(up.up())
		...
		// down
		if(pos.getY() > EvlieyeClient.MC.world.getBottomY() && canGoThrough(down)
	...
	private boolean canGoThrough(BlockPos pos)
	{
		...
		if(!EvlieyeClient.MC.world.isChunkLoaded(pos))
			return false;
