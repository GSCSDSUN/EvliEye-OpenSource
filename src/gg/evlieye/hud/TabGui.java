...
import gg.evlieye.other_features.TabGuiOtf;
...
public final class TabGui implements KeyPressListener
{
	...
	private final TabGuiOtf tabGuiOtf =
		EvlieyeClient.INSTANCE.getOtfs().tabGuiOtf;
	...
	public TabGui()
	{
		EvlieyeClient.INSTANCE.getEventManager().add(KeyPressListener.class,
			this);
		...
		features.addAll(EvlieyeClient.INSTANCE.getHax().getAllHax());
		features.addAll(EvlieyeClient.INSTANCE.getCmds().getAllCmds());
		features.addAll(EvlieyeClient.INSTANCE.getOtfs().getAllOtfs());
	...
	private void updateSize()
	{
		...
		for(Tab tab : tabs)
		{
			int tabWidth = EvlieyeClient.MC.textRenderer.getWidth(tab.name) + 10;
			if(tabWidth > width)
				width = tabWidth;
		}
		height = tabs.size() * 10;
	...
	public void render(MatrixStack matrixStack, float partialTicks)
	{
		...
		ClickGui gui = EvlieyeClient.INSTANCE.getGui();
		...
		for(int i = 0; i < tabs.size(); i++)
		{
			...
			EvlieyeClient.MC.textRenderer.draw(matrixStack, tabName, 2, textY,
				txtColor);
			textY += 10;
		}
		GL11.glEnable(GL11.GL_BLEND);
		...
		if(tabOpened)
		{
			...
			for(int i = 0; i < tab.features.size(); i++)
			{
				...
				EvlieyeClient.MC.textRenderer.draw(matrixStack, fName, 2,
					tabTextY, txtColor);
				tabTextY += 10;
			}
			GL11.glEnable(GL11.GL_BLEND);
	...
	private static final class Tab
	{
		...
		public void updateSize()
		{
			...
			{
				int fWidth =
					EvlieyeClient.MC.textRenderer.getWidth(feature.getName())
						+ 10;
				if(fWidth > width)
					width = fWidth;
			}
		...
		private void onEnter()
		{
			...
			TooManyHaxHack tooManyHax =
				EvlieyeClient.INSTANCE.getHax().tooManyHaxHack;
