	...
	protected void onResize()
	{
		...
		okButton = new ButtonWidget(width / 2 - 151, height - 65, 149, 18,
			Text.literal("OK"), b -> {
				if(choosingKey)
				{
					...
					String oldCommands = EvlieyeClient.INSTANCE.getKeybinds()
						.getCommands(selectedKey);
					if(oldCommands != null)
						newCommands = oldCommands + " ; " + newCommands;
					
					EvlieyeClient.INSTANCE.getKeybinds().add(selectedKey,
						newCommands);
					
					EvlieyeClient.INSTANCE.getNavigator()
						.addPreference(parent.getFeature().getName());
					EvlieyeClient.MC.setScreen(parent);
				}else
				{
					choosingKey = true;
		...
		addDrawableChild(ButtonWidget
			.builder(Text.literal("Cancel"),
				b -> EvlieyeClient.MC.setScreen(parent))
	...
	protected void onUpdate()
	{
		...
		if(choosingKey)
		{
			...
			if(!selectedKey.equals("key.keyboard.unknown"))
			{
				...
				String commands =
					EvlieyeClient.INSTANCE.getKeybinds().getCommands(selectedKey);
				if(commands != null)
				{
					text +=
