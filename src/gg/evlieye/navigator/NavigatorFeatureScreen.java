...
import gg.evlieye.keybinds.Keybind;
...
public final class NavigatorFeatureScreen extends NavigatorScreen
{
		...
		if(hasPrimaryAction)
		{
			primaryButton =
				ButtonWidget.builder(Text.literal(primaryAction), b -> {
					...
					EvlieyeClient.INSTANCE.getNavigator()
		...
		if(!possibleKeybinds.isEmpty())
		{
			...
			ButtonData addKeybindButton =
					...
					area.y + getStringHeight(text) - 7, 12, 8, "+", 0x00ff00)
				{
					...
					{
						// add keybind
						EvlieyeClient.MC.setScreen(new NavigatorNewKeybindScreen(
			...
			for(Keybind keybind : EvlieyeClient.INSTANCE.getKeybinds()
				.getAllKeybinds())
			{
	...
	{
		// popups
		if(EvlieyeClient.INSTANCE.getGui().handleNavigatorPopupClick(x, y,
			button))
			return;
		...
		// component settings
		EvlieyeClient.INSTANCE.getGui().handleNavigatorMouseClick(
	...
	{
		EvlieyeClient.INSTANCE.getGui().handleMouseRelease(x, y, button);
	}
	...
	{
		window.close();
		EvlieyeClient.INSTANCE.getGui().handleMouseClick(Integer.MIN_VALUE,
			Integer.MIN_VALUE, 0);
	}
