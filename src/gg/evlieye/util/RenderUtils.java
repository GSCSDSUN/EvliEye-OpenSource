...
public enum RenderUtils
{
	...
	public static void scissorBox(int startX, int startY, int endX, int endY)
	{
		...
		int bottomY = EvlieyeClient.MC.currentScreen.height - endY;
		double factor = EvlieyeClient.MC.getWindow().getScaleFactor();
	...
	public static void applyCameraRotationOnly()
	{
		...
		// Camera camera =
		// EvlieyeClient.MC.getBlockEntityRenderDispatcher().camera;
	...
	{
		Camera camera = EvlieyeClient.MC.getBlockEntityRenderDispatcher().camera;
		if(camera == null)
			return Vec3d.ZERO;
		...
		int y, boolean large)
	{
		...
		EvlieyeClient.MC.getItemRenderer().renderInGuiWithOverrides(matrixStack,
			renderStack, 0, 0);
		...
		if(stack.isEmpty())
		{
			...
			TextRenderer fr = EvlieyeClient.MC.textRenderer;
